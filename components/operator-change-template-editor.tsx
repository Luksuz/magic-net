...
import { 
  getEditableOperatorChangeTemplate, 
  saveEditableOperatorChangeTemplate, 
  getOriginalOperatorChangeTemplate, 
  resetToOriginalOperatorChangeTemplate,
  initializeOperatorChangeTemplates
} from '@/lib/template-service'

export function OperatorChangeTemplateEditor() {
  ...
  useEffect(() => {
    ...
    const fetchTemplate = async () => {
      try {
        console.log('Dohvaćanje predloška za promjenu operatera...')
        
        // Initialize templates if needed
        try {
          await initializeOperatorChangeTemplates();
        } catch (initError) {
          console.warn('Could not initialize operator change templates:', initError);
        }
        
        const templateData = await getEditableOperatorChangeTemplate();
        console.log('Predložak za promjenu operatera dohvaćen:', templateData)
        // Set the template HTML
        if (templateData && templateData.html) {
          console.log("Predložak za promjenu operatera je pronađen")
          ...
          console.log("Predložak za promjenu operatera nije pronađen")
          // Fallback to empty template
          setFullTemplate('');
        }
      } catch (error) {
        console.error('Greška pri dohvaćanju predloška za promjenu operatera:', error);
        // Fallback to empty template
        setFullTemplate('');
      }
    };
  ...
  const handleSave = async () => {
    try {
      ...
      await saveEditableOperatorChangeTemplate(templateToSave);
      
      toast({
        title: 'Uspjeh',
        description: 'Predložak za promjenu operatera je spremljen',
      });
    } catch (error) {
      toast({
        title: 'Greška',
        description: 'Nije moguće spremiti predložak za promjenu operatera',
        variant: 'destructive',
      });
      console.error('Failed to save operator change template:', error);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleReset = async () => {
    try {
      ...
      await resetToOriginalOperatorChangeTemplate();
      
      // Get the original template from database to update the editor
      ...
      toast({
        title: 'Uspjeh',
        description: 'Predložak za promjenu operatera je vraćen na izvornu verziju',
      });
    } catch (error) {
      toast({
        title: 'Greška',
        description: 'Nije moguće vratiti izvorni predložak za promjenu operatera',
        variant: 'destructive',
      });
      console.error('Failed to reset to original operator change template:', error);
    } finally {
  ...
  return (
    <Card className="w-full">
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle>Uređivanje predloška za promjenu operatera</CardTitle>
          ...
          <AlertDialog open={showResetConfirm} onOpenChange={setShowResetConfirm}>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Ovo će vratiti predložak za promjenu operatera na izvornu verziju. Sve nesačuvane promjene bit će izgubljene. 
          ...
          <div className="space-y-4">
            <div>
              <p className="mb-2 text-sm text-slate-500">
                {`Uredi HTML predložak za promjenu operatera. Koristi varijable unutar uglatih zagrada poput [USER_NAME], [OIB], [PHONE_NUMBER], itd.`}
              </p>
              <Textarea
                rows={25}
                value={fullTemplate}
