...
export function ProfileEditor() {
  ...
  const [agreementNumber, setAgreementNumber] = useState<number | null>(profile?.agreement_number || null)
  const [activationFees, setActivationFees] = useState<number[]>(profile?.activation_fees || [])
  const [userNumber, setUserNumber] = useState<number | null>(profile?.user_number || null)
  ...
  const handleAgreementNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value.trim();
    if (value === '') {
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Ovaj broj će se koristiti kao zadnji broj u nazivu i naslovu ugovora.
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="activation-fees">Dostupne naknade za aktivaciju</Label>
            {isEditing ? (
              <MultiSelect
                options={activationFeeOptions}
                selectedValues={activationFees.map(fee => 
                  activationFeeOptions.find(option => option.value === fee) || { value: fee, label: `${fee / 100} EUR` }
                )}
                onChange={(selected) => setActivationFees(selected.map(item => item.value))}
                placeholder="Odaberite dostupne naknade"
              />
            ) : (
              <div className="p-2 border rounded">
                {profile.activation_fees?.length 
                  ? profile.activation_fees.map(fee => `${(fee / 100).toFixed(2)} EUR`).join(', ') 
                  : 'Nije postavljeno'}
              </div>
            )}
            <p className="text-sm text-muted-foreground mt-1">
              Ove naknade će biti dostupne za odabir prilikom kreiranja novog ugovora.
            </p>
          </div>
        </div>
      </CardContent>
